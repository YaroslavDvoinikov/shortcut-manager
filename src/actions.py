    ...
    match command:
        ...
        case 6:
            ...
            if not audio_record_started:
                ...
                filename = (
                        dir_path
                        / f"Screenshot-{now.year}-{now.month:02d}-{now.day:02d}-{now.hour:02d}{now.minute:02d}{now.second:02d}.wav"
                )
...
def info(command: int):
    text = ["Run an executable", "Take screenshot", "Start recording screen", "Power off", "Reboot", "Log out",
            "Record audio"]
    return text[command]
